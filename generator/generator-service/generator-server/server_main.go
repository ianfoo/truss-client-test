...
func main() {
	...
	*/
	...
	{
		//generateDuration := duration.With(metrics.Field{Key: "method", Value: "Generate})"
		//generateLogger := log.NewContext(logger).With("method", "Generate)")

		generateEndpoint = svc.MakeGenerateEndpoint(service)
		//generateEndpoint = opentracing.TraceServer(tracer, "Generate)")(generateEndpoint)
		//generateEndpoint = svc.EndpointInstrumentingMiddleware(generateDuration)(generateEndpoint)
		//generateEndpoint = svc.EndpointLoggingMiddleware(generateLogger)(generateEndpoint)
	}
