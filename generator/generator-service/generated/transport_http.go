...
func MakeHTTPHandler(ctx context.Context, endpoints Endpoints, logger log.Logger) http.Handler {
	...
	m.Handle("/generate", httptransport.NewServer(
		ctx,
		endpoints.GenerateEndpoint,
		HttpDecodeLogger(DecodeHTTPGenerateZeroRequest, logger),
...
// DecodeHTTPGenerateZeroRequest is a transport/http.DecodeRequestFunc that
// decodes a JSON-encoded generate request from the HTTP request
// body. Primarily useful in a server.
func DecodeHTTPGenerateZeroRequest(_ context.Context, r *http.Request) (interface{}, error) {
	...
	pathParams, err := PathParams(r.URL.Path, "/generate")
	_ = pathParams
	if err != nil {
	...
	NameGenerateStr := queryParams["name"]
	NameGenerate := NameGenerateStr
	// TODO: Better error handling
	if err != nil {
		fmt.Printf("Error while extracting NameGenerate from query: %v\n", err)
		fmt.Printf("queryParams: %v\n", queryParams)
		return nil, err
	}
	req.Name = NameGenerate
...
// DecodeHTTPGenerate is a transport/http.DecodeResponseFunc that decodes
// a JSON-encoded GenerateResponse response from the HTTP response body.
// If the response has a non-200 status code, we will interpret that as an
// error and attempt to decode the specific error message from the response
// body. Primarily useful in a client.
func DecodeHTTPGenerateResponse(_ context.Context, r *http.Response) (interface{}, error) {
...
// EncodeHTTPGenerateZeroRequest is a transport/http.EncodeRequestFunc
// that encodes a generate request into the various portions of
// the http request (path, query, and body).
func EncodeHTTPGenerateZeroRequest(_ context.Context, r *http.Request, request interface{}) error {
