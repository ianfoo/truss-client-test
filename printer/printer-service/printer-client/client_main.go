...
import (
	...
	grpcclient "github.com/hasian/truss-client-test/printer/printer-service/generated/client/grpc"
	httpclient "github.com/hasian/truss-client-test/printer/printer-service/generated/client/http"
	clientHandler "github.com/hasian/truss-client-test/printer/printer-service/handlers/client"
...
func main() {
	...
	var (
		...
		method = flag.String("method", "print", "print")
	)

	var (
		flagNamePrint = flag.String("print.name", "", "")
	)
	flag.Parse()
	...
	case "print":
		...
		var err error
		NamePrint := *flagNamePrint
		request, err := clientHandler.Print(NamePrint)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error calling clientHandler.Print: %v\n", err)
			os.Exit(1)
		}

		v, err := service.Print(context.Background(), request)
		if err != nil {
			fmt.Fprintf(os.Stderr, "Error calling service.Print: %v\n", err)
			os.Exit(1)
		}
